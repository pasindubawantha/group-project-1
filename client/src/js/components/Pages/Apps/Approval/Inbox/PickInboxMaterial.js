...
@connect((store) => {
	return {//props
		url: store.url,
		list: store.lists.approvalsInboxMaterial,
		userId: store.user.userEmployeeId
	}
},)
//class for pickvinores
export default class PickInboxMaterial extends React.Component {
	componentWillMount(){
		this.props.dispatch(fetchApprovalList('approvalsInboxMaterial', 'inbox/material/' + this.props.userId))
	}
	render(){
		var { list } = this.props
		if(list.downloaded){
			...
			var nextURL = this.props.url.slice(0, this.props.url.length - 1).concat({ name: "View Pedning Request", viewID: "ApprovalViewInboxMaterial"})
			return (
