...
@connect((store) => {
	return {//props
		url: store.url,
		list: store.lists.approvalsInboxPayment,
		userId: store.user.userEmployeeId
	}
},)
//class for pickvinores
export default class PickInboxPayment extends React.Component {
	componentWillMount(){
		this.props.dispatch(fetchApprovalList('approvalsInboxPayment', 'inbox/payment/' + this.props.userId))
	}
	render(){
		var { list } = this.props
		if(list.downloaded){
			...
			var nextURL = this.props.url.slice(0, this.props.url.length - 1).concat({ name: "View Pedning Request", viewID: "ApprovalViewInboxDenied"})
			return (
